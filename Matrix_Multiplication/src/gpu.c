...
void copy_gather(double *B, double *B_loc,double *B_mult,unsigned long long int N_loc,unsigned long long  int N,unsigned long long  int ncol, int *nrows, int *displs);
int* get_nrows_displs(unsigned long long int N, unsigned long long int ncol, int size);
...
*/
void gpu_blas(unsigned long long int N, char *fname, char *datacsv)
{
    ...
    double t2_gpu = MPI_Wtime(); // data created

    double *d_A, *d_B_mult, *d_C;
    ...
    double t3_gpu = MPI_Wtime(); // data created
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        unsigned long long int ncol = N/size + 1*(i < N%size);
        unsigned long long int offset = ncol*i + N%size*(i >= N%size);
        int* nrows = get_nrows_displs(N, ncol, size);

        // get the rows of B to be multiplied in a single matrix and copy them to GPU
        copy_gather(&B[offset],B_loc,B_mult, N_loc, N, ncol, nrows, &nrows[size]);
    ...
    double gpu_init = t3_gpu-t2_gpu;
    // init_time += gpu_init;

    double global = t6-t1;
    double computational_global, communication_global, init_global, global_max, gpu_init_global;
    ...
    MPI_Reduce(&gpu_init,&gpu_init_global,1,MPI_DOUBLE,MPI_MAX,0,MPI_COMM_WORLD);

    if (rank == 0)
        write_csv_gpu(2,size,N,global_max,computational_global,communication_global,init_global,gpu_init,datacsv);
